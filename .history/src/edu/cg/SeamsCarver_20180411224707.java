...
public class SeamsCarver extends ImageProcessor {
	...
	private void removeSeam() {
		...
		for(int x = this.inHeight - 1; x > -1; --x){
			//TODO: fix xIndicets
			seam[x] = this.xIndices[x][minRowValue];
			shiftImgLeft(x , minRowValue);
			minRowValue = this.minPathsArrax[x][minRowValue];
		}
	}

	private void shiftImgLeft(int y, int minRowValue) {
	}

	private int findMinRow() {
		int minRow = 0;
		for (int x = 0; x < inWidth - seamsRemoved; x++) {
			if(M_Matrix[inHeight - 1][x] <  M_Matrix[inHeight - 1][minRow]){
				minRow = x;
			}
		}
		return minRow;
