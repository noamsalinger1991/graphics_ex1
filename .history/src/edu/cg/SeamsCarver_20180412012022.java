...
public class SeamsCarver extends ImageProcessor {
	...
	//calculate the m matrix acording to working img position
	...
	//calculate the magnitude of a given pixel
	...
	//find the min path for a given pixel
	private MinPixelTupple findMinPath(int y, int x) {
		...
		cLeft = (x > 0) ? cLeft + getMagnitudeDiff(y - 1, x, y, x - 1) : Long.MAX_VALUE;

		cRight = (x + 1 < this.inWidth - this.seamsRemoved) ? cRight + getMagnitudeDiff(y - 1, x, y, x + 1)
	...
	//get Magnitude Diff of two given pixels
	private long getMagnitudeDiff(int y1, int x1, int y2, int x2) {
		return (long) Math.abs(this.greyImg[y1][x1] - this.greyImg[y2][x2]);
	}

	//remove the seam from the working img to avoid seams being picked more then once
