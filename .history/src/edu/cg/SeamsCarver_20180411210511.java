...
public class SeamsCarver extends ImageProcessor {
	private class MinPixelTupple{
		float minPixelValue;
		int min_x_value;
		MinPixelTupple(float minPixelValue, int min_x_value){
	...
	private MinPixelTupple findMinCost(int x, int y) {
		...
		0;
		// add posible new neighbors(subtrection is done to avoid overflow of float)
		cLeft = (x > 0) ? 
		...
		if(leftSum < verticalSum){
			if(leftSum < rightSum){
				min = new MinPixelTupple(x - 1 , leftSum)
			}
		}
