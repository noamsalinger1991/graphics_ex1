...
public class SeamsCarver extends ImageProcessor {
	...
	private void removeSeam() {
		int minRowVal= findMinRow();
		int[] seam = new int[this.inHeight]; 
		for(int y = this.inHeight - 1; y > -1; --y){
			//TODO: fix xIndicets
			seam[y] = this.xIndices[y][minRowVal];
			shiftRowLeft(y , minRowVal);
			minRowVal = this.minPathsArray[y][minRowVal];
		}
	}

	private void shiftRowLeft(int y, int minRowValue) {
	}

	private int findMinRow() {
		int minRowVal = 0;
		for (int y = 0; y < inWidth - seamsRemoved; y++) {
			if(M_Matrix[inHeight - 1][y] <  M_Matrix[inHeight - 1][minRowVal]){
				minRowVal = y;
			}
		}
		return minRowVal;
